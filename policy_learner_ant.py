import os
import locale  # 통화 문자열 포맷을 위함.
import logging  # 학습 과정 정보를 기록하기 위함.
import numpy as np
...
class PolicyLearner:
    ...
    def __init__(self, stock_code, chart_data, environment, training_data=None,
                 ...
                 delayed_reward_threshold=.05, lr=0.0001):
    ...
    def reset(self):  # 학습 데이터를 처음부터 읽게 하기 위함.
        self.sample = None
        self.training_data_idx = -1

    def fit(
        self, num_epoches=1000, max_memory=10, balance=10000000,
        discount_factor=0, start_epsilon=.5, learning=True):  # 책에선 주식투자의 경우 지연 보상 발생 시점의 바로 직전의 행동이 그 이전 행동보다 중요하다 볼 수 없다고 간주하여 discount_factor=0
        ...
        for epoch in range(num_epoches):    
            ...
            logger.info("[Epoch %s/%s]\tEpsilon:%.4f\t#Expl.:%d/%d\t"   # 에포크 수행 회수, 탐험률, 탐험 회수, 
                        "#Buy:%d\t#Sell:%d\t#Hold:%d\t"  # 매수 횟수, 매도 횟수, 관망 횟수
                        "#Stocks:%d\tPV:%s\t"  # 보유 주식 수, 포트폴리오 가치
                        "POS:%s\tNEG:%s\tLoss:%10.6f" % (  # 긍정적 학습 횟수, 부정적 학습 횟수, 학습 손실
